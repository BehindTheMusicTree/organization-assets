...
Copy **`playground/.env.example`** to **`playground/.env`** and set **every** key before **`npm run playground`** or **`npm run build`**. **`scripts/run-playground.mjs`** merges **`playground/.env`** into the environment for the root **`npm run build`** (shell overrides file). **`scripts/assert-org-url.mjs`** requires **`ORG_URL`**, **`ORG_GITHUB_SPONSOR_BUTTON_URL`**, and all playground social URLs (same names as **`publish.yml`** / **`.env.example`**). **tsup** inlines **`ORG_URL`**, **`ORG_GITHUB_SPONSOR_BUTTON_URL`**, and those social keys into published **`dist/`** (defaults for **`TheMusicTree*Link`**, **`GithubSponsorButton`**, and **`Social*Link`**). Re-run root **`npm run build`** after editing **`playground/.env`** so **`node_modules/…/dist/`** picks up new values. If the playground UI stays blank after changing **`dist/`**, remove **`playground/node_modules/.vite`** and restart the dev server.
